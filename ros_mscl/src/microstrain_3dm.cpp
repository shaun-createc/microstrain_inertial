...
void Microstrain::run()
{
  ...
  try
  {
    ...
    while(ros::ok())
    {
      ...
      {
        MicrostrainCommon::parse_mip_packet(packet);
      }
...
void Microstrain::parse_and_publish_imu_packet(const mscl::MipDataPacket &packet)
{
  //Parse the packet
  MicrostrainCommon::parse_imu_packet(packet);
...
void Microstrain::parse_and_publish_filter_packet(const mscl::MipDataPacket &packet)
{
  //Parse the packet
  MicrostrainCommon::parse_filter_packet(packet);
  ...
  {
    if(m_publish_gnss_aiding_status[i] && m_gnss_aiding_status_received[i])
...
void Microstrain::parse_and_publish_gnss_packet(const mscl::MipDataPacket &packet, int gnss_id)
{
  //Range-check id
  if(gnss_id >= NUM_GNSS)
    return;

  //Get current time according to ROS
  const auto& ros_time = ros::Time::now();

  //Parse the packet
  MicrostrainCommon::parse_gnss_packet(packet, ros_time, gnss_id);
...
void Microstrain::parse_and_publish_rtk_packet(const mscl::MipDataPacket& packet)
{
  //Parse the packet
  MicrostrainCommon::parse_rtk_packet(packet);
